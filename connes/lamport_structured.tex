...
Let $h_t:=f(\cdot+t)-f(\cdot)$.  By Step~3, $h_t=0$ a.e.\ on $J$, so $h_t=0$ in $L^1(J)$.
Hence
\[
...
\Stepjust{Cover $I$ by overlapping compact subintervals $J_n\Subset I$ (each chosen with length $>2\delta_n$
for the appropriate $\delta_n$) and apply Steps~1--9 to each.  On overlaps, the a.e.\ constants must agree,
so $\1_B$ is a.e.\ constant on all of $I$.  Since $\1_B\in\{0,1\}$ a.e., either $m(B)=0$ or $m(I\setminus B)=0$.}
...
\Stepjust{For $0<t<2L$, $\|\widetilde 1-S_t\widetilde 1\|_2^2=m(I\,\Delta\,(I+t))=2t>0$
...
\Stepjust{Since $A_\lambda\ge 0$, the Laplace-transform identity
\[
...
A bounded operator commuting with the bounded selfadjoint $R$ commutes with every bounded Borel function of $R$,
in particular with its spectral projections, hence with the spectral measure $E_R$.
Since $R=\varphi(A_\lambda)$ with $\varphi(\lambda)=(\lambda+1)^{-1}$, a Borel bijection
$(0,\infty)\to(0,1)$, the spectral measures satisfy $E_{A_\lambda}(\Delta)=E_R(\varphi(\Delta))$ for Borel $\Delta$.
Thus $P$ commutes with $E_{A_\lambda}(\Delta)$ for all Borel $\Delta$, and therefore
$P$ commutes with $A_\lambda^{1/2}=\int_0^\infty \mu^{1/2}\,dE_{A_\lambda}(\mu)$
