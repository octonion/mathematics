...
for the Restricted Weil Quadratic Form\\[6pt]
{\large (Lamport Structured-Proof Edition)}}
\author{}
\date{}
...
We then prove the Markov (normal contraction) property and a translation-invariance lemma which yields irreducibility from the archimedean continuum of shifts.
Finally, we show that the quadratic form is closed and that its associated selfadjoint operator has compact resolvent, using a logarithmic lower bound on the Fourier symbol together with the Kolmogorov-Riesz compactness criterion. From this we deduce that the ground-state eigenvalue is simple and its eigenfunction can be chosen strictly positive and, by inversion symmetry, even.
...
Substituting $y\mapsto y/a$ (with $d^*(y/a)=d^*y$) gives
...
\Substepjust{As $t\to\infty$, $w(t)=e^{t/2}/(2\sinh t)\sim e^{-t/2}$, so
$2e^{-t/2}w(t)\sim 2e^{-t}$, which is integrable.}
...
\Substepjust{Near $t=0$: $w(t)\sim (2t)^{-1}$ and $e^{-t/2}-1\sim -t/2$, so the
integrand is $\sim -1/2$, which is bounded.  On the compact interval $[0,2L]$ with
this bounded behavior, the integral converges absolutely.}
...
+c_\infty(\lambda)\|G\|_{L^2(I)}^2$.}
\Stepjust{Combine Steps~8.1, 7, and 9, noting $\|g\|_2^2=\|G\|_{L^2(I)}^2$ by the change
of variables.}
...
\Substep{Integrate Step~1.2 over $\R$ with respect to $du$:
\[
\int_\R|\Phi(\widetilde G(u))-\Phi(\widetilde G(u-t))|^2\,du
\le\int_\R|\widetilde G(u)-\widetilde G(u-t)|^2\,du.
\]}
\Substepjust{Integrate both sides of the pointwise inequality from Step~1.2.}

\Substep{Rewrite using Step~1.1 and the definition of $S_t$: this is precisely
...
\Stepjust{By Definition~\ref{def:E}, $\mathcal E_\lambda(G)$ is the integral of
...
(all coefficients $\ge 0$).  Step~1 shows each summand decreases (or stays the same)
...
\Step{The identity extends to $|t|<\delta$: $f(u+t)=f(u)$ for a.e.\ $u\in J$, for all $t\in(-\delta,\delta)$.}
\Stepjust{Step~2 handles $t\in(0,\delta)$.  For $t\in(-\delta,0)$, replace $t$ by $-t\in(0,\delta)$
in Step~2 to get $f(u-t)=f(u)$ a.e.\ on $J$, which after the substitution $u\mapsto u+t$
(and noting $J+t\subset I$ for $|t|<\delta$, same argument as Step~2.3) yields $f(u)=f(u+t)$ a.e.\ on appropriate sets.
At $t=0$ the identity is trivial.}

\Step{Define the mollification $f_\eta:=f*\rho_\eta$ on $J_\eta:=\{u\in J:\mathrm{dist}(u,\R\setminus J)>\eta\}$,
where $\rho_\eta(s)=\eta^{-1}\rho(s/\eta)$ for a standard nonneg.\ mollifier $\rho$ supported in $(-1,1)$ with $\int\rho=1$, and $0<\eta<\delta/2$.}
\Stepjust{Standard construction.  $f_\eta\in C^\infty(J_\eta)$ by standard properties of convolution with $C_c^\infty$ kernels.}

\Step{$f_\eta$ is translation-invariant on $J_\eta$ for shifts $|t|<\delta/2$:
$f_\eta(u+t)=f_\eta(u)$ for $u\in J_\eta$.}

\Substep{For $u\in J_\eta$ and $s\in\supp\rho_\eta\subset(-\eta,\eta)$: $u-s\in J$.}
\Substepjust{$|u-s-u|=|s|<\eta<\mathrm{dist}(u,\R\setminus J)$ by definition of $J_\eta$.}

\Substep{$f_\eta(u+t)=\int_\R f(u+t-s)\rho_\eta(s)\,ds
=\int_\R f(u-s)\rho_\eta(s)\,ds=f_\eta(u)$.}
\Substepjust{By Fubini and Step~5.1, for each $s\in\supp\rho_\eta$, $u-s\in J$ and
$|t|<\delta/2<\delta$, so Step~3 gives $f((u-s)+t)=f(u-s)$ for a.e.\ such $u-s$.
Hence $f(u+t-s)=f(u-s)$ a.e., and integration against $\rho_\eta(s)\,ds$ yields the identity.}

\Step{$f_\eta$ is constant on $J_\eta$.}
\Stepjust{$J_\eta$ is a connected open interval (nonempty for small $\eta$) and $f_\eta\in C^\infty(J_\eta)$
is invariant under all translations of size $<\delta/2$ (Step~5).
A smooth function on a connected open subset of $\R$ that is invariant under arbitrarily small translations
has zero derivative everywhere: for any $u\in J_\eta$, $f_\eta'(u)=\lim_{t\to 0}(f_\eta(u+t)-f_\eta(u))/t=0$.
Hence $f_\eta$ is constant.}

\Step{$f=\1_B$ is a.e.\ constant on $J$.}
\Stepjust{As $\eta\downarrow 0$, $f_\eta\to f$ in $L^1(J)$ (standard mollification convergence theorem).
Since each $f_\eta$ is constant on $J_\eta$ and $J_\eta\nearrow J^\circ=J$, the limit $f$ is
a.e.\ equal to a constant on~$J$.}

\Step{$f=\1_B$ is a.e.\ constant on~$I$.}
\Stepjust{The compact subinterval $J\Subset I$ was arbitrary (Step~1).  Cover $I$ by
a countable increasing sequence $J_1\subset J_2\subset\cdots$ of compact subintervals
with $\bigcup_n J_n=I$.  By Step~7, $\1_B$ is a.e.\ constant on each $J_n$.  Since
successive $J_n$'s overlap, the constant must be the same on all of them.
Hence $\1_B$ is a.e.\ constant on~$I$.}

\Step{$m(B)=0$ or $m(I\setminus B)=0$.}
\Stepjust{Since $\1_B\in\{0,1\}$ a.e., the only possible constants are $0$ and $1$.
If $\1_B=0$ a.e.\ on $I$, then $m(B)=0$.  If $\1_B=1$ a.e.\ on $I$, then $m(I\setminus B)=0$.}
...
\]}
\Stepjust{All integrands are nonneg.\ ($w(t)\ge 0$, $\sin^2\ge 0$, $|\widehat\phi|^2\ge 0$, $(\log p)p^{-m/2}\ge 0$).
...
is continuous in $\xi$ by dominated convergence (the integrand is bounded by $w(t)$, which is integrable).
...
\Substepjust{The integral $\int_0^{2L}w(t)\sin^2(\xi t/2)\,dt\le\int_0^{2L}w(t)\,dt<\infty$
(since $w(t)\sim (2t)^{-1}$ near $0$ and the domain is bounded).
...
\Stepjust{A nonneg.\ quadratic form is closed iff its domain equipped with the graph norm is complete.
This is Step~3.}
...
\Substepjust{By the mean value theorem applied pointwise:
$|\phi(u)-\phi(u-t)|=|t\phi'(\xi_u)|\le|t|\cdot\|\phi'\|_\infty$ for some $\xi_u$ between $u-t$ and $u$.
But we can be more precise: $\phi(u)-\phi(u-t)=\int_0^t\phi'(u-s)\,ds$, so
$|\phi(u)-\phi(u-t)|^2\le t^2\|\phi'\|_\infty^2$.
Integrating and using that $\phi$ has compact support:
$\|\phi-S_t\phi\|_2^2\le t^2\|\phi'\|_2^2$ (by a Cauchy--Schwarz refinement, or
simply note $\le t^2\cdot\mathrm{meas}(\mathrm{supp}(\phi)+[-|t|,|t|])\cdot\|\phi'\|_\infty^2$;
more directly, Plancherel gives $\|\phi-S_t\phi\|_2^2=\frac{1}{2\pi}\int 4\sin^2(\xi t/2)|\widehat\phi(\xi)|^2\,d\xi\le t^2\frac{1}{2\pi}\int\xi^2|\widehat\phi(\xi)|^2\,d\xi=t^2\|\phi'\|_2^2$).}

\Substep{$\int_0^{2L}w(t)t^2\,dt<\infty$.}
\Substepjust{Near $t=0$: $w(t)\sim(2t)^{-1}$, so $w(t)t^2\sim t/2$, which is integrable.
The domain $[0,2L]$ is bounded, so the integral is finite.}
...
\Stepjust{The restriction of a closed form to a closed subspace is closed.  More explicitly:
if $G_n\to G$ in $L^2(I)$ and $\mathcal E_\lambda(G_n-G_m)\to 0$, then $\widetilde G_n\to\widetilde G$
...
\Stepjust{$\sinh t=\frac{e^t-e^{-t}}{2}\le\frac{e^t}{2}\cdot 2t/(2t/(e^t-e^{-t})\cdot 2)$---more directly,
$\sinh t=\sum_{n=0}^\infty\frac{t^{2n+1}}{(2n+1)!}\le t\sum_{n=0}^\infty\frac{t^{2n}}{(2n)!}
=t\cosh t\le te^t$.}
...
$N\le\frac{t_0|\xi|}{2\pi}+\frac{1}{4}+1$.  The condition $|\xi|\ge 4\pi/t_0$ ensures $N\ge 1$.
...
\Step{$\sum_{n=0}^{N-1}\log\frac{2\pi n+3\pi/2}{2\pi n+\pi/2}\ge c'\log N-C$ for absolute constants $c',C>0$.}
...
\Stepjust{Chain Steps~2, 5, 6, and 7: $\psi_\lambda(\xi)\ge 4c_0\cdot\frac{1}{2}\cdot(c'\log N-C)
\ge 2c_0c'(\log|\xi|+O(1))-2c_0C$.  Set $c_1:=2c_0c'$ and absorb the $O(1)$ into~$c_2$.}
...
\begin{remark}
See, e.g., Lieb--Loss~\cite{LiebLossAnalysis} for a proof of Theorem~\ref{thm:KR}.
\end{remark}
...
\begin{structuredproof}
\Step{For $\phi\in\mathcal K_M$ and $|h|\le 1$:
...
Corollary~\ref{cor:tail} bounds $\int\log(2+|\xi|)|\widehat\phi|^2$ by $aM+bM=C(M,L)$.}
...
\Stepjust{The map $\phi\mapsto\phi|_I$ is a continuous surjection $H_I\to L^2(I)$
(indeed, an isometry).  Continuous images of relatively compact sets are relatively compact.
...
\Stepjust{By the representation theorem for densely defined, closed, lower-bounded symmetric forms
(Kato~\cite[Thm.~VI.2.1]{KatoPerturbation}; in the Dirichlet-form setting,
...
\Stepjust{$P$ commutes with $T(t)=e^{-tA_\lambda}$ for all $t>0$ (Step~2).
By the spectral theorem, $P$ commutes with every bounded Borel function of $A_\lambda$.
In particular, $P$ commutes with the spectral projections of $A_\lambda$, hence with
$A_\lambda^{1/2}$ (which is defined via the spectral theorem).}
...
If $u\in\mathcal D(A_\lambda^{1/2})$, then $A_\lambda^{1/2}Pu=PA_\lambda^{1/2}u\in L^2(I)$ (Step~3),
...
\Substepjust{$G=PG+QG$ and $A_\lambda^{1/2}$ commutes with $P$ and $Q$ (Step~3).}
...
\Substepjust{Pythagorean theorem (Step~5.3), then $PA_\lambda^{1/2}G=A_\lambda^{1/2}PG$ (Step~3),
then the form identity (Step~5.1) applied to $PG$ and $QG$ (which lie in $\mathcal D(\mathcal E_\lambda)$
by Step~4).}
...
\Substepjust{$w(t)\sim(2t)^{-1}$ near $0$, so $w(t)\cdot 2t\sim 1$ near $0$: integrable.
On $[0,2L]$ (bounded), the integral is finite.}
...
\Substepjust{Combine Steps~2.3 and~2.4.}

\Step{Apply~\eqref{eq:Einvariance} with $G\equiv 1$.
...
\Substep{$\mathcal E_\lambda(1)=\mathcal E_\lambda(\1_B)+\mathcal E_\lambda(\1_{B^c})$ (from Step~1 with $G=1$).}
...
If $S$ is positivity improving, then $\min\sigma(A)$ is a simple eigenvalue
...
Ouhabaz~\cite[Ch.~1, \S\S1.4--1.5]{OuhabazHeatEq}), the associated semigroup $T(t)$
...
\Stepjust{$A_\lambda$ is selfadjoint and lower bounded ($A_\lambda\ge 0$).
By the spectral theorem, $e^{-zA_\lambda}$ is bounded and holomorphic on
$\{z\in\C:\Re z>0\}$.  In particular, $T(t)=e^{-tA_\lambda}$ extends to a
...
improving (Step~4).  The conclusion follows.}
...
\Substepjust{Unitarity of $R$.}

\Substep{$\|\widetilde G-S_{-t}\widetilde G\|_2=\|\widetilde G-S_t\widetilde G\|_2$.}
\Substepjust{$\|\phi-S_{-t}\phi\|_2=\|S_t\phi-\phi\|_2=\|\phi-S_t\phi\|_2$ (norm is symmetric).
More formally: $\|\phi-S_{-t}\phi\|_2^2=\int|\phi(u)-\phi(u+t)|^2\,du$; substituting $v=u+t$:
...
$\langle A_\lambda Ru,v\rangle=\mathcal E_\lambda(Ru,v)$.}
\Substepjust{$Ru\in\mathcal D(A_\lambda)$ (since $R$ preserves $\mathcal D(\mathcal E_\lambda)\supset\mathcal D(A_\lambda)$, and the form identity extends).  The form identity
$\langle A_\lambda w,v\rangle=\mathcal E_\lambda(w,v)$ for $w\in\mathcal D(A_\lambda)$, $v\in\mathcal D(\mathcal E_\lambda)$.  Set $w=Ru$.}

\Substep{$\mathcal E_\lambda(Ru,v)=\mathcal E_\lambda(u,R^{-1}v)=\mathcal E_\lambda(u,Rv)$.}
\Substepjust{Invariance of $\mathcal E_\lambda$ under $R$ (Step~4) implies, by polarization,
...
\Substep{Combining: $\langle A_\lambda Ru,v\rangle=\langle RA_\lambda u,v\rangle$ for all $v\in\mathcal D(\mathcal E_\lambda)$.}
\Substepjust{Steps~5.1, 5.2, 5.3.}

\Substep{$A_\lambda Ru=RA_\lambda u$.}
\Substepjust{$\mathcal D(\mathcal E_\lambda)$ is dense in $L^2(I)$
(Proposition~\ref{prop:closedI}).
Step~5.4 says $A_\lambda Ru-RA_\lambda u$ is orthogonal to a dense subspace, hence $=0$.}
...
\bibitem{OuhabazConvex1996}
E.-M.~Ouhabaz.
\newblock Invariance of closed convex sets and domination criteria for semigroups.
\newblock \emph{Potential Anal.} \textbf{5} (1996), no.~6, 611--625.
\newblock DOI:~10.1007/BF00275797.
