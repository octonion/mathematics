...
Since the family of functions $\{e^{-t\lambda}\}_{t>0}$ separates points on $[0,\infty)$,
a bounded operator commuting with every $e^{-tA_\lambda}$ must commute with the
spectral measure $E(\cdot)$ of $A_\lambda$ (by the spectral theorem and a monotone-class
or Stone--Weierstrass argument), hence with every bounded Borel function of $A_\lambda$.
In particular, $P$ commutes with
$A_\lambda^{1/2}=\int_0^\infty \lambda^{1/2}\,dE(\lambda)$.}
