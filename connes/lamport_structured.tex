...
\Stepjust{$A_\lambda$ is selfadjoint and lower bounded ($A_\lambda\ge 0$), hence m-sectorial
...
(a)~$\sigma(A_\lambda)\ne\emptyset$;
(b)~$\lambda_1:=\min\sigma(A_\lambda)$ exists and is an eigenvalue;
(c)~the corresponding eigenfunction is strictly positive a.e.;
(d)~$\lambda_1$ is a simple eigenvalue (multiplicity~one).}
