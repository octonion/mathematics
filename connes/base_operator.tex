...
of the Restricted Weil Quadratic Form Base Operator}
\author{}
\date{}
...
Note that $c_p(\lambda)<0$ in Lemma~\ref{lem:prime-energy}, so the sign of the total additive constant $C_\lambda$ is not controlled here.
What it does establish is the Markov, irreducible, and compact-resolvent structure of the nonnegative difference-energy part $\mathcal E_\lambda$, and hence the simplicity, strict positivity, and evenness of the ground-state eigenfunction---properties that are unaffected by adding a scalar multiple of $\|G\|_2^2$.
\end{remark}
...
There exists a unique selfadjoint operator $L_\lambda\ge 0$ on $L^2(I)$ associated to the closed form
$\mathcal E_\lambda$ (Proposition~\ref{prop:closedI}) in the sense of the representation theorem for closed forms.
Moreover, $L_\lambda$ has compact resolvent.
\end{theorem}
...
$\mathcal E_\lambda(G)=\|L_\lambda^{1/2}G\|_2^2$.}
...
$\mathcal{D}(\mathcal{E}_\lambda)=\mathcal{D}(L_\lambda^{1/2})$ and the identity
$\mathcal{E}_\lambda(G)=\|L_\lambda^{1/2}G\|_2^2$.
Proposition~\ref{prop:closedI} verified that $\mathcal E_\lambda$ satisfies all hypotheses.}

\Step{$(L_\lambda+1)^{-1}$ is compact on $L^2(I)$.}

\Substep{Let $\{f_n\}$ be bounded in $L^2(I)$: $\|f_n\|_2\le C$.  Set $u_n:=(L_\lambda+1)^{-1}f_n$.}
\Substepjust{$L_\lambda+1$ is invertible because $L_\lambda\ge 0$, so $L_\lambda+1\ge 1>0$.}

\Substep{$u_n\in\mathcal D(L_\lambda)\subset\mathcal D(\mathcal E_\lambda)$ and $(L_\lambda+1)u_n=f_n$.}
\Substepjust{Definition of the resolvent.}

\Substep{$\mathcal E_\lambda(u_n)+\|u_n\|_2^2=\langle f_n,u_n\rangle$.}
\Substepjust{Take the $L^2$ inner product of $(L_\lambda+1)u_n=f_n$ with $u_n$:
$\langle L_\lambda u_n,u_n\rangle+\|u_n\|_2^2=\langle f_n,u_n\rangle$.
By the form identity, $\langle L_\lambda u_n,u_n\rangle=\mathcal E_\lambda(u_n)$.}
...
\Substep{$(L_\lambda+1)^{-1}$ maps bounded sequences to sequences with convergent subsequences,
hence is compact.}
...
$L^2(B)$ is invariant under $T(t)=e^{-tL_\lambda}$.
...
\Substepjust{$P^*=P$ and $T(t)^*=T(t)$ (selfadjointness of $L_\lambda$ implies selfadjointness of $e^{-tL_\lambda}$).
...
\Step{$P$ commutes with $(L_\lambda+\alpha)^{-1}$ for every $\alpha>0$.}
\Stepjust{By the Laplace-transform formula for $C_0$-semigroups
(Engel--Nagel~\cite[Cor.\,II.1.11]{EngelNagel}):
\[
(L_\lambda+\alpha)^{-1}=\int_0^\infty e^{-\alpha t}\,e^{-tL_\lambda}\,dt
\]
holds as a Bochner integral in $\mathcal B(L^2(I))$
(convergence: $\|e^{-\alpha t}T(t)\|\le e^{-\alpha t}$, integrable for $\alpha>0$;
$\alpha$ lies in the resolvent set of $-L_\lambda$ since $\sigma(L_\lambda)\subset[0,\infty)$).
By Step~2, $P$ commutes with $e^{-tL_\lambda}$ for every $t\ge 0$, hence commutes with the Bochner integral.}

\Step{$P$ commutes with $L_\lambda^{1/2}$.}
\Stepjust{Let $R:=(L_\lambda+1)^{-1}$, which is bounded and selfadjoint.
...
Since $R=\varphi(L_\lambda)$ with $\varphi(\mu)=(\mu+1)^{-1}$, a continuous strictly
decreasing bijection $[0,\infty)\to(0,1]$, the spectral measures satisfy
$E_{L_\lambda}(\Delta)=E_R(\varphi(\Delta))$ for Borel $\Delta\subset[0,\infty)$.
Thus $PE_{L_\lambda}(\Delta)=E_{L_\lambda}(\Delta)P$ for all Borel $\Delta$.
\emph{Domain preservation:}
For $u\in\mathcal D(L_\lambda^{1/2})$,
$\int_0^\infty\mu\,d\|E_{L_\lambda}(\mu)Pu\|^2
=\int_0^\infty\mu\,d\|PE_{L_\lambda}(\mu)u\|^2
\le\int_0^\infty\mu\,d\|E_{L_\lambda}(\mu)u\|^2
=\|L_\lambda^{1/2}u\|^2<\infty$,
so $Pu\in\mathcal D(L_\lambda^{1/2})$.
\emph{Commutativity on the domain:}
$L_\lambda^{1/2}Pu=\int_0^\infty\mu^{1/2}\,dE_{L_\lambda}(\mu)Pu
=\int_0^\infty\mu^{1/2}\,PE_{L_\lambda}(\mu)u\,d\mu
=P\int_0^\infty\mu^{1/2}\,dE_{L_\lambda}(\mu)u=PL_\lambda^{1/2}u$
(interchange of $P$ with the spectral integral is justified since $P$ is bounded
and commutes with each $E_{L_\lambda}(\Delta)$).}
...
\Stepjust{$\mathcal D(\mathcal E_\lambda)=\mathcal D(L_\lambda^{1/2})$.
If $u\in\mathcal D(L_\lambda^{1/2})$, then $L_\lambda^{1/2}Pu=PL_\lambda^{1/2}u\in L^2(I)$ (Step~4),
so $Pu\in\mathcal D(L_\lambda^{1/2})$.  Similarly for $Qu=(I-P)u$.}

\Step{$\mathcal E_\lambda(G)=\mathcal E_\lambda(PG)+\mathcal E_\lambda(QG)$ for $G\in\mathcal D(\mathcal E_\lambda)$.}

\Substep{$\mathcal E_\lambda(G)=\|L_\lambda^{1/2}G\|_2^2$.}
\Substepjust{Form identity from the representation theorem (Step~1 of Theorem~\ref{thm:operator}).}

\Substep{$L_\lambda^{1/2}G=L_\lambda^{1/2}PG+L_\lambda^{1/2}QG=PL_\lambda^{1/2}G+QL_\lambda^{1/2}G$.}
\Substepjust{$G=PG+QG$ and $L_\lambda^{1/2}$ commutes with $P$ and $Q$ (Step~4).}

\Substep{$PL_\lambda^{1/2}G$ and $QL_\lambda^{1/2}G$ are orthogonal in $L^2(I)$.}
\Substepjust{$\langle Pv,Qv\rangle=\langle Pv,(I-P)v\rangle=\langle Pv,v\rangle-\langle Pv,Pv\rangle
=\langle P^2v,v\rangle-\|Pv\|^2=\|Pv\|^2-\|Pv\|^2=0$.  Apply with $v=L_\lambda^{1/2}G$.}

\Substep{$\|L_\lambda^{1/2}G\|^2=\|PL_\lambda^{1/2}G\|^2+\|QL_\lambda^{1/2}G\|^2
=\|L_\lambda^{1/2}PG\|^2+\|L_\lambda^{1/2}QG\|^2
=\mathcal E_\lambda(PG)+\mathcal E_\lambda(QG)$.}
\Substepjust{Pythagorean theorem (Step~6.3), then $PL_\lambda^{1/2}G=L_\lambda^{1/2}PG$ (Step~4),
...
Assume Theorem~\ref{thm:operator}. Then $T(t)=e^{-tL_\lambda}$ is irreducible.
\end{corollary}
...
\subsection{Application to $L_\lambda$}

\begin{proposition}[Positivity improving and simple ground state for $L_\lambda$]
\label{prop:groundstate}
Assume Theorem~\ref{thm:operator}. Then:
\begin{enumerate}
\item The semigroup $T(t)=e^{-tL_\lambda}$ is positivity preserving (Markovian).
\item $T(t)$ is irreducible.
\item $T(t)$ is holomorphic.
\end{enumerate}
Consequently $T(t)$ is positivity improving, and the lowest eigenvalue of $L_\lambda$ is simple with a
...
$\|T(t)f-f\|_2^2=\int_0^\infty|e^{-t\mu}-1|^2\,d\|E_{L_\lambda}(\mu)f\|^2\to 0$
as $t\to 0^+$ by dominated convergence.
Moreover, $L_\lambda$ is selfadjoint and lower bounded ($L_\lambda\ge 0$), hence m-sectorial
with numerical range in $[0,\infty)$.
By Kato~\cite[Ex.~IX.1.25]{KatoPerturbation} (backed by
Thm.\,IX.1.24), $e^{-zL_\lambda}$ is bounded and holomorphic on
$\{z\in\C:\Re z>0\}$ with $\|e^{-zL_\lambda}\|\le 1$.
In particular, $T(t)=e^{-tL_\lambda}$ extends to a
...
\Step{The lowest eigenvalue of $L_\lambda$ is simple, with a strictly positive a.e.\ eigenfunction.}
\Stepjust{Apply Theorem~\ref{thm:principal-simple}: $L_\lambda$ is selfadjoint, lower bounded,
has compact resolvent (Theorem~\ref{thm:operator}), and $T(t)=e^{-tL_\lambda}$ is positivity
improving (Step~4).
All four conclusions of Theorem~\ref{thm:principal-simple}
(ATG~\cite[Prop.~2.4]{ATGStrictPos2020}) hold:
(a)~$\sigma(L_\lambda)\ne\emptyset$;
(b)~$\mu_0:=\min\sigma(L_\lambda)$ exists and is an eigenvalue
...
Consequently, $L_\lambda$ commutes with $R$.
\end{proposition}
...
\Step{$L_\lambda$ commutes with $R$.}

\Substep{For $u\in\mathcal D(L_\lambda)$ and $v\in\mathcal D(\mathcal E_\lambda)$:
...
\Substep{$\mathcal E_\lambda(u,Rv)=\langle L_\lambda u,Rv\rangle=\langle RL_\lambda u,v\rangle$.}
\Substepjust{Form identity: $\mathcal E_\lambda(u,Rv)=\langle L_\lambda u,Rv\rangle$ (since $u\in\mathcal D(L_\lambda)$
and $Rv\in\mathcal D(\mathcal E_\lambda)$).
Then $\langle L_\lambda u,Rv\rangle=\langle R^*L_\lambda u,v\rangle=\langle RL_\lambda u,v\rangle$
since $R^*=R$ ($R$ is selfadjoint: $R=R^{-1}=R^*$).}

\Substep{Combining: $\mathcal E_\lambda(Ru,v)=\langle RL_\lambda u,v\rangle$ for all $v\in\mathcal D(\mathcal E_\lambda)$.}
\Substepjust{Steps~5.1 and 5.2.}

\Substep{$Ru\in\mathcal D(L_\lambda)$ and $L_\lambda Ru=RL_\lambda u$.}
\Substepjust{By the representation theorem (Kato~\cite[Thm.~VI.2.1]{KatoPerturbation}),
$w\in\mathcal D(L_\lambda)$ if and only if there exists $h\in L^2(I)$ such that
$\mathcal E_\lambda(w,v)=\langle h,v\rangle$ for all $v\in\mathcal D(\mathcal E_\lambda)$,
in which case $L_\lambda w=h$.
Step~5.3 provides exactly this with $w=Ru$ and $h=RL_\lambda u\in L^2(I)$.}
...
\begin{structuredproof}
\Step{Define $\psi^\sharp:=R\psi$.  Then $\psi^\sharp$ is an eigenfunction of $L_\lambda$ for the same
eigenvalue $\mu_0:=\min\sigma(L_\lambda)$.}
\Stepjust{$L_\lambda\psi=\mu_0\psi$.  Since $L_\lambda R=RL_\lambda$ (Proposition~\ref{prop:reflection}):
$L_\lambda\psi^\sharp=L_\lambda R\psi=RL_\lambda\psi=R(\mu_0\psi)=\mu_0 R\psi=\mu_0\psi^\sharp$.}
