...
\author{Mathematical Foundations Series}
\date{}
...
\]
while Solution 1 derives the answer directly from the exact finite-$N$ law. In the Beta-law routes, the desired probability is the Beta tail
\[
...
\section{Solution 1: Exact state probabilities and explicit finite summation}

This gives the exact finite-$N$ distribution and then evaluates the majority probability explicitly.
...
\subsection*{Step 3: write the finite-$N$ majority probability as a polynomial sum}
Since
\[
X_N>\frac12
\quad\Longleftrightarrow\quad
...
\frac{12\,(k+1)(k+2)(N-k+1)}{(N+1)(N+2)(N+3)(N+4)}.
\]
The denominator is constant in $k$, so the problem reduces to summing the cubic polynomial
\[
(k+1)(k+2)(N-k+1).
\]
Expanding,
...
(k+1)(k+2)(N-k+1)
=
-k^3+(N-2)k^2+(3N+1)k+(2N+2).
\]

\subsection*{Step 4: evaluate the sum in the even case}
Let $N=2m$. Then
\[
X_{2m}>\frac12
\quad\Longleftrightarrow\quad
...
so
\[
\mathbb{P}\!\left(X_{2m}>\frac12\right)
=
\frac{12}{(2m+1)(2m+2)(2m+3)(2m+4)}
\sum_{k=m}^{2m}(k+1)(k+2)(2m-k+1).
\]
Using the standard formulas for $\sum k$, $\sum k^2$, and $\sum k^3$, one finds
\[
\sum_{k=m}^{2m}(k+1)(k+2)(2m-k+1)
=
\frac{11m^4+58m^3+109m^2+86m+24}{12}.
\]
Therefore
\[
\mathbb{P}\!\left(X_{2m}>\frac12\right)
=
\frac{11m^2+25m+12}{4(2m+1)(2m+3)}.
\]

\subsection*{Step 5: evaluate the sum in the odd case}
Let $N=2m+1$. Then
\[
X_{2m+1}>\frac12
\quad\Longleftrightarrow\quad
...
so
\[
\mathbb{P}\!\left(X_{2m+1}>\frac12\right)
=
\frac{12}{(2m+2)(2m+3)(2m+4)(2m+5)}
\sum_{k=m+1}^{2m+1}(k+1)(k+2)(2m+2-k).
\]
Again using the standard power-sum formulas, one obtains
\[
\sum_{k=m+1}^{2m+1}(k+1)(k+2)(2m+2-k)
=
\frac{11m^4+63m^3+133m^2+121m+36}{12}.
\]
Hence
\[
\mathbb{P}\!\left(X_{2m+1}>\frac12\right)
=
\frac{11m^2+41m+36}{4(2m+3)(2m+5)}.
\]

\subsection*{Step 6: take the limit}
In the even case,
\[
\mathbb{P}\!\left(X_{2m}>\frac12\right)
=
...
Therefore
\[
P_\infty=\lim_{N\to\infty}\mathbb{P}\!\left(X_N>\frac12\right)=\frac{11}{16}.
\]
